...
class Genome(object):
    ...
    def build_phenotype(self):
        ...
        return int(self._input_nodes),int(len(self._node_genes)-self._input_nodes-self._output_nodes),int(self._output_nodes),\
            c_src,c_trg,c_wts,\
            float(Config.max_weight-Config.min_weight), [float(Config.min_out_weight),float(Config.max_out_weight)]
    ...
    def mutate(self):
        ...
        if random.random() < Config.prob_addnode:
            self._mutate_add_node()
        elif random.random() < Config.prob_addconn:
                ...
                for out_node in self._node_genes[self._input_nodes:]:
                    if (in_node.id, out_node.id) not in self._connection_genes.keys():
                        ...
                        if count == n: # Connection to create
                            weight = float("{0:.4f}".format(random.gauss(0, Config.weight_stdev)))
...
class ConnectionGene(object):
    ...
    def mutate(self):
        ...
        if random.random() < Config.prob_mutate_weight:
            if random.random() < Config.prob_uni_mut_weight:
                # Replace the weight uniformly
                self.__uniform_replace_weight()
            else:
                self.__mutate_weight()

        if random.random() <  Config.prob_togglelink:
            self.enable()
    ...
    def __mutate_weight(self):
        self._weight += float("{0:.4f}".format(random.gauss(0,1)*Config.weight_mutation_power))

        if self._weight > Config.max_weight:
            self._weight = float("{0:.4f}".format(Config.max_weight))
        elif self._weight < Config.min_weight:
            self._weight = float("{0:.4f}".format(Config.min_weight))

    def __uniform_replace_weight(self):
        self._weight = float("{0:.4f}".format(random.uniform(Config.min_weight, Config.max_weight)))
